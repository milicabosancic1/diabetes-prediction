...
# ---------- izbor familije modela (na osnovu outputs/summary.json) ----------
    ...
    for s in summary:
        if best is None:
            ...
            if (s.get("test_f1",0) > best.get("test_f1",0)) or (
                ...
                best = s
    name = best["model"]
    if name.startswith("KNN"): return "KNN"
    if name.startswith("LogReg"): return "LogReg"
    if name.startswith("MLP"): return "MLP"
    if name.startswith("NaiveBayes"): return "NaiveBayes"
    return "LogReg"
...
def predict_single(
    ...
    """
    user_input: dict sa ključevima kao u FEATURES (Pregnancies, Glucose, ... Age).
    Vraca: {score, threshold, label, family}
    """
    # validacija ključeva
    ...
    # pripremi dataset + stats (median/mean/std) — BEZ feature engineeringa
    data = prepare_dataset(csv_path, seed=42)

    # istreniraj model
    model = _instantiate_model(fam)
    if hasattr(model, "fit"): model.fit(data.X_train, data.y_train)
    elif hasattr(model, "train"): model.train(data.X_train, data.y_train)
    ...
    # median imput sa train-medijanima
