import os, io, sys, urllib.request, pandas as pd
...
def try_read_csv_from_text(txt: str) -> pd.DataFrame:
    ...
    try:
        df = pd.read_csv(buf)
        if df.shape[1] != 9:
            buf.seek(0); df = pd.read_csv(buf, header=None, names=COLUMNS)
    except Exception:
        buf.seek(0); df = pd.read_csv(buf, header=None, names=COLUMNS)
    if df.shape[1] != 9: raise ValueError("Unexpected number of columns")
    df.columns = COLUMNS
    return df
